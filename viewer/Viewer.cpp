...
namespace Pivot {
    ...
    Viewer::Viewer(std::filesystem::path const & dirname): Viewer() {
        if (! LoadDirectory(dirname)) {
            spdlog::critical(
                "Failed to create Viewer without a valid directory");
            std::exit(EXIT_FAILURE);
        }

        m_Loader.Load(m_Dirname, m_FrameCount, m_Objects);

        for (auto & object : m_Objects) { object->UploadBuffers(0, true); }
    ...
    void Viewer::Tick(float deltaTime) {
        ...
        PassConstants passConstants = {
            .Transform =
                m_Camera->GetTransform(App::Get()->GetWindow()->GetAspect()),
            .LightIntensity =
                m_Appearance.LightColor * m_Appearance.LightIntensity * .1f,
            .LightDirection = Coordinates::SphericalToCartesian(
                1.f,
                glm::radians(90.f) - m_Appearance.LightAltitude,
                m_Appearance.LightAzimuth),
            .AmbientCoeff = m_Appearance.EnvironColor
                * m_Appearance.EnvironIntensity * .001f,
        ...
        for (auto const & layer : m_ObjectLayers) {
            for (auto const & object : layer) { object->Draw(); }
        }
    }
    ...
    void Viewer::OnKeyDown(Key key, ModifierFlags mods) {
        switch (key) {
        case Key::Left:
            if (! m_Animation.Playing && m_Animation.CurrentFrame >= 1.f)
                m_Animation.CurrentFrame -= 1.f;
            break;
        case Key::Right:
            if (! m_Animation.Playing
                && m_Animation.CurrentFrame < m_AvailFrameCount - 1)
    ...
    void Viewer::OnMouseMove(float xPos, float yPos) {
        ...
        if (Input::IsMouseButtonDown(MouseButton::Left)) {
            m_Camera->Rotate(
                (mousePos - m_CameraInfo.LastMousePos)
                / (1.f * App::Get()->GetWindow()->GetHeight()));
    ...
    bool Viewer::LoadDirectory(std::filesystem::path const & dirname) {
        ...
        { // Load the description file
            ...
            if (! fin) {
                spdlog::error(
                    "Failed to open \"{}\"",
                    (m_Dirname / "description.yaml").string());
                return false;
            }
        ...
        // Load the dimension
        m_Dimension = m_Description["Dimension"]
            ? m_Description["Dimension"].as<std::uint32_t>()
            : 3;
        App::Get()->GetWindow()->SetTitle(fmt::format(
            "Pivot Viewer ({}D) - {}", m_Dimension, m_Dirname.string()));
        auto radius =
            m_Description["Radius"] ? m_Description["Radius"].as<float>() : 1.f;
        if (m_Dimension == 2) {
            m_Camera = std::make_unique<OrthoCamera>(glm::vec2(0), radius * 2);
            m_InitialCamera = std::make_unique<OrthoCamera>(
                *reinterpret_cast<OrthoCamera *>(m_Camera.get()));
        } else {
            m_Camera = std::make_unique<OrbitCamera>(glm::vec3(0), radius);
            m_InitialCamera = std::make_unique<OrbitCamera>(
                *reinterpret_cast<OrbitCamera *>(m_Camera.get()));
        }

        // Load objects
        if (! m_Description["Objects"]
            || ! m_Description["Objects"].IsSequence()) {
            spdlog::error("Failed to find any object in description.yaml");
            return false;
        }
        for (auto const & node : m_Description["Objects"]) {
            auto object = std::make_unique<ViewObject>(
                m_Objects.size(), node, m_Dimension);
            m_ObjectLayers[static_cast<std::size_t>(object->GetMaterial().Mode)]
    ...
    void Viewer::ExportModels() {
        ...
        std::uint32_t const frameBegin =
            m_ExportModels.AllFramesSelected ? 0 : m_CurrentFrame;
        std::uint32_t const frameEnd = m_ExportModels.AllFramesSelected
            ? m_AvailFrameCount
            : m_CurrentFrame + 1;
        for (std::size_t i = 0; i < m_Objects.size(); i++) {
            ...
            fmt::print("\r");
            spdlog::info(
                "Completed exportation of \"{}\"", m_Objects[i]->GetName());
        }
    }
    ...
    void Viewer::RenderSideBars() {
        ...
        if (m_MenuBarVisible && ImGui::BeginMainMenuBar()) {
            ...
            if (ImGui::BeginMenu("Control")) {
                if (ImGui::MenuItem(
                        m_Animation.Playing ? "Stop" : "Play", "Ctrl+P")) {
            ...
            if (ImGui::BeginMenu("View")) {
                ...
                if (ImGui::MenuItem(
                        "Appearance", "F10", m_Appearance.Visible)) {
        ...
        if (m_StatusBarVisible && ImGuiEx::BeginMainStatusBar()) {
            if (m_AvailFrameCount == m_FrameCount) {
                ...
                ImGui::Text(
                    "Loading... %.1f%%",
                    100.f * m_AvailFrameCount / m_FrameCount);
            }
            // Show frame number
            ImGui::PushFont(ImGui::GetIO().Fonts->Fonts[1]);
            ImGuiEx::TextRightAligned(
                "%u/%u", m_CurrentFrame, m_AvailFrameCount - 1);
            ImGui::PopFont();
    ...
    void Viewer::RenderPopups() {
        ...
        if (ImGui::BeginPopupModal(
                "Export Models",
                &m_ExportModels.Popup,
                ImGuiWindowFlags_AlwaysAutoResize)) {
            ImGui::Text("Output Directory");
            if (ImGuiEx::InputText(
                    "##Dirname",
                    &m_ExportModels.DirnameStr,
                    ImGuiInputTextFlags_EnterReturnsTrue)) {
                m_ExportModels.Dirname =
                    std::filesystem::absolute(m_ExportModels.DirnameStr);
                m_ExportModels.DirnameStr = m_ExportModels.Dirname.string();
            }
            ImGui::SameLine();
            if (ImGui::Button("Browse")) {
                auto newDirname = FileDialog::PickFolder(
                    App::Get()->GetWindow(), m_ExportModels.Dirname);
                if (! newDirname.empty()) {
            ...
            ImGui::BeginListBox(
                "##Objects",
                { -FLT_MIN, 5 * ImGui::GetTextLineHeightWithSpacing() });
            for (std::size_t i = 0; i < m_Objects.size(); i++) {
                ImGui::Checkbox(
                    m_Objects[i]->GetDisplayName().data(),
                    reinterpret_cast<bool *>(&m_ExportModels.Exported[i]));
            }
            ImGui::EndListBox();
            ImGui::Spacing();
            if (ImGui::RadioButton(
                    "Current frame", ! m_ExportModels.AllFramesSelected)) {
                m_ExportModels.AllFramesSelected = false;
            }
            ImGui::SameLine();
            if (ImGui::RadioButton(
                    "All frames", m_ExportModels.AllFramesSelected)) {
            ...
            ImGui::Separator();
            {
                ...
                float buttonWidth2 = ImGui::CalcTextSize("Cancel").x
                    + style.FramePadding.x * 2.f;
                float widthNeeded =
                    buttonWidth1 + style.ItemSpacing.x + buttonWidth2;
                ImGui::SetCursorPosX(
    ...
    void Viewer::RenderAnimationPanel() {
        ...
        if (ImGui::Begin(
            ...
            if (ImGui::InputInt(
                    "Frame no.",
                    &m_Animation.FrameNumber,
                    1,
                    10,
                    ImGuiInputTextFlags_EnterReturnsTrue)) {
                m_Animation.FrameNumber = std::clamp<int>(
                    m_Animation.FrameNumber, 0, m_AvailFrameCount - 1);
                m_Animation.CurrentFrame = m_Animation.FrameNumber;
            }
            if (ImGui::InputFloat(
                    "Frame rate", &m_Animation.FrameRate, 1.f, 10.f, "%.1f")) {
    ...
    void Viewer::RenderAppearancePanel() {
        ...
        if (ImGui::Begin(
            ...
            if (ImGui::ColorEdit3(
                    "Background", glm::value_ptr(m_Appearance.Background))) {
                Renderer::SetClearColor(m_Appearance.Background);
            }
            if (auto msCnt = App::Get()->GetWindow()->GetMultisampleCount();
                msCnt > 1
                && ImGui::Checkbox(
                    fmt::format("Anti-aliasing ({}x MSAA)", msCnt).c_str(),
                    &m_Appearance.Multisampled)) {
                Renderer::SetMultisampled(m_Appearance.Multisampled);
            }
            ...
            if (m_Dimension == 3) {
                if (ImGui::Checkbox(
                        "Back-face culling", &m_Appearance.BackFaceCulled)) {
                    Renderer::SetBackFaceCulled(m_Appearance.BackFaceCulled);
                }
                if (ImGui::RadioButton(
                        "Vertex normal", m_Appearance.VertexNormalUsed)) {
                    m_Appearance.VertexNormalUsed = true;
                }
                ImGui::SameLine();
                if (ImGui::RadioButton(
                        "Face normal", ! m_Appearance.VertexNormalUsed)) {
                ...
                ImGui::ColorEdit3(
                    "Color##Light", glm::value_ptr(m_Appearance.LightColor));
                ImGui::SliderFloat(
                    "Intensity##Light",
                    &m_Appearance.LightIntensity,
                    0,
                    200,
                    "%.1f",
                ...
                ImGui::ColorEdit3(
                    "Color##Environ",
                    glm::value_ptr(m_Appearance.EnvironColor));
                ImGui::SliderFloat(
                    "Intensity##Environ",
                    &m_Appearance.EnvironIntensity,
                    0,
                    200,
                    "%.1f",
                    ImGuiSliderFlags_NoInput
    ...
    void Viewer::RenderCameraPanel() {
        ...
        if (ImGui::Begin(
            ...
            if (m_Dimension == 2) {
                ...
                ImGui::BulletText(
                    "Press Ctrl+Backspace to reset the parameters");
            } else {
                ImGui::BulletText(
    ...
    void Viewer::RenderObjectsPanel() {
        ...
        if (ImGui::Begin(
                "Objects",
                &m_ObjectsInfo.Visible,
                ImGuiWindowFlags_AlwaysAutoResize)) {
            if (ImGui::BeginCombo(
                    "Name",
                    m_Objects[m_ObjectsInfo.CurIdx]->GetDisplayName().data())) {
                for (std::size_t i = 0; i < m_Objects.size(); i++) {
                    bool const selected = (i == m_ObjectsInfo.CurIdx);
                    if (ImGui::Selectable(
                            m_Objects[i]->GetDisplayName().data(), selected))
            ...
            if (m_ObjectsInfo.CurIdx < 9) {
                ImGui::BeginDisabled();
                ImGui::Text(
                    "Global shortcut keys: Ctrl+%d",
                    static_cast<int>(m_ObjectsInfo.CurIdx) + 1);
                ImGui::EndDisabled();
            }
            ...
            ImGui::BulletText(
                "Shader name: \"%s\";",
                ViewShaderPool::GetName(curObj->GetViewShader()).data());
            if (curObj->IsAnimated()) {
                ImGui::BulletText(
                    "Animation: %s topology;",
                    curObj->IsTopoFixed() ? "invariant" : "variant");
            } else {
                ImGui::BulletText("Animation: unavailable;");
            }
            if (curObj->IsIndexed()) {
                ImGui::BulletText(
                    "Statistics: %u vertices, %u indices.",
                    curObj->GetVertexCount(m_CurrentFrame),
                    curObj->GetIndexCount(m_CurrentFrame));
            } else {
                ImGui::BulletText(
                    "Statistics: %u vertices.",
                    curObj->GetVertexCount(m_CurrentFrame));
            }
            ...
            ImGui::Combo(
                "Blending", &blending, "Opaque\0Cutout\0Transparent\0Fade\0");
            ImGui::EndDisabled();
            ...
            case ViewShader::Heatmap3D_Points: {
                ImGui::SliderFloat(
                    "Scale",
                    &curMat.RadScale,
                    .1f,
    ...
    void Viewer::RegisterGlobalShortcuts() {
        ...
        // Camera shortcuts
        if (ImGui::Shortcut(
                ImGuiKey_ModCtrl | ImGuiKey_Backspace,
                0,
                ImGuiInputFlags_RouteGlobalLow)) {
            m_Camera->SetAs(*m_InitialCamera);
        }
        ...
        for (std::size_t i = 0; i < m_Objects.size() && i < 9; i++) {
            if (ImGui::Shortcut(
                    ImGuiKey_ModCtrl | ImGuiKey_1 + i,
                    0,
                    ImGuiInputFlags_RouteGlobalLow)) {
                m_Objects[i]->GetMaterial().Visible =
                    ! m_Objects[i]->GetMaterial().Visible;
            }
        }
    }
    ...
    std::unique_ptr<App> CreateApp(CmdLineArgs args) {
        ...
        try {
            cxxopts::Options argParser(
                "viewer",
                "The Pivot viewer of animations for research in computer graphics");
