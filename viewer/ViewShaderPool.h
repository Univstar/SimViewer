...
namespace Pivot {
    ...
    public:
        static std::string_view GetName(ViewShader viewShader) {
            return c_Names[static_cast<std::size_t>(viewShader)];
        }
        static Shader * GetShader(ViewShader viewShader) {
            return s_Shaders[static_cast<std::size_t>(viewShader)].get();
        }
    ...
    private:
        ...
        static inline std::array<
            ...
            static_cast<std::size_t>(ViewShader::Count)>
            s_Shaders;

        static inline std::unordered_map<std::string, ViewShader> const
            c_NameToViewShader = {
                ...
                { "heatmap_3d_triangles",           ViewShader::Heatmap3D },
                {     "heatmap_3d_lines",           ViewShader::Heatmap3D },
                {    "heatmap_2d_points",    ViewShader::Heatmap2D_Points },
                {    "heatmap_3d_points",    ViewShader::Heatmap3D_Points },
