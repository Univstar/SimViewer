...
namespace YAML {
    template<> struct convert<Pivot::PrimitiveType> {
    ...
    template<> struct convert<Pivot::BlendMode> {
...
namespace Pivot {
    ViewObject::ViewObject(
        std::size_t nodeId, YAML::Node const & node, std::uint32_t dimension) {
        ...
        constexpr auto SetValue =
            []<typename T>(T & val, YAML::Node const & node) {
        ...
        static constexpr auto c_PrimitiveNames =
        ...
        m_ViewShader = ViewShaderPool::GetViewShader(fmt::format(
            "{}_{}d_{}",
            shaderName,
            dimension,
            c_PrimitiveNames[static_cast<std::size_t>(primitive)]));
        if (m_ViewShader == ViewShader::Count) { // invalid shader name
            spdlog::error(
                "Failed to find the shader \"{}_{}d_{}\"",
                shaderName,
                dimension,
                c_PrimitiveNames[static_cast<std::size_t>(primitive)]);
            m_ViewShader = ViewShaderPool::GetViewShader(fmt::format(
        ...
        if (shaderName == "heatmap") { m_AttribFlags |= AttribFlagBits::Heat; }
        ...
        if (m_Indexed) {
            m_VertexArray =
                Renderer::CreateIndexedVertexArray(layout, primitive);
        } else {
    ...
    bool ViewObject::LoadNewFrameData(
        std::filesystem::path const & dirname, bool initial) {
        if (! m_Animated && ! initial) return true;
        std::ifstream fin(dirname / (m_Name + ".out"), std::ios::binary);
        if (! fin) {
            spdlog::error(
                "Failed to open \"{}\"",
                (dirname / (m_Name + ".out")).string());
            return false;
        }
        FrameData frameData;
        ...
        if (m_AttribFlags & AttribFlagBits::Heat) {
            ...
            std::span<float const> heats(
                reinterpret_cast<float const *>(frameData.Heats.data()),
                vtxCount);
            auto const [itMin, itMax] =
                std::minmax_element(heats.begin(), heats.end());
            m_Material.HeatMin = std::min(m_Material.HeatMin, *itMin);
            m_Material.HeatMax = std::max(m_Material.HeatMax, *itMax);
        }
        if ((! m_TopoFixed || initial)
            && (m_AttribFlags & AttribFlagBits::TexCoord)) {
    ...
    void ViewObject::UploadBuffers(std::uint32_t frame, bool initial) {
        if (initial) {
            ...
            if (m_AttribFlags & AttribFlagBits::Position)
                m_VertexArray->GetBufferByName("Positions")
                    ->Upload(m_FramesData[posFrame].Positions);
            if (m_AttribFlags & AttribFlagBits::Normal)
                m_VertexArray->GetBufferByName("Normals")->Upload(
                    m_FramesData[posFrame].Normals);
            if (m_AttribFlags & AttribFlagBits::Heat)
                m_VertexArray->GetBufferByName("Heats")->Upload(
                    m_FramesData[posFrame].Heats);
            if (m_AttribFlags & AttribFlagBits::TexCoord)
                m_VertexArray->GetBufferByName("TexCoords")
                    ->Upload(m_FramesData[topoFrame].TexCoords);
            if (m_AttribFlags & AttribFlagBits::Radius)
                m_VertexArray->GetBufferByName("Radii")->Upload(
                    m_FramesData[topoFrame].Radii);
            if (m_Indexed) {
                auto indexedVA =
                    reinterpret_cast<IndexedVertexArray *>(m_VertexArray.get());
                indexedVA->GetIndexBuffer()->Upload(
                    m_FramesData[topoFrame].Indices);
            }
        } else if (m_Animated && frame != m_CurrentFrame) {
            m_CurrentFrame = frame;
            if (m_AttribFlags & AttribFlagBits::Position)
                m_VertexArray->GetBufferByName("Positions")
                    ->Upload(m_FramesData[frame].Positions);
            if (m_AttribFlags & AttribFlagBits::Normal)
                m_VertexArray->GetBufferByName("Normals")->Upload(
                    m_FramesData[frame].Normals);
            if (m_AttribFlags & AttribFlagBits::Heat)
                m_VertexArray->GetBufferByName("Heats")->Upload(
                    m_FramesData[frame].Heats);
            if (! m_TopoFixed && (m_AttribFlags & AttribFlagBits::TexCoord))
                m_VertexArray->GetBufferByName("TexCoords")
                    ->Upload(m_FramesData[frame].TexCoords);
            if (m_AttribFlags & AttribFlagBits::Radius)
                m_VertexArray->GetBufferByName("Radii")->Upload(
                    m_FramesData[frame].Radii);
            if (! m_TopoFixed && m_Indexed) {
                auto indexedVA =
                    reinterpret_cast<IndexedVertexArray *>(m_VertexArray.get());
                indexedVA->GetIndexBuffer()->Upload(
                    m_FramesData[frame].Indices);
            }
        }
    }
    ...
    void ViewObject::Draw() const {
        ...
        case ViewShader::Heatmap2D:
            ...
            shader->SetUniform(
                "u_HeatScale",
                m_Material.HeatMin == m_Material.HeatMax
                    ? 0.f
                    : 1.f / (m_Material.HeatMax - m_Material.HeatMin));
            break;
